...
// 6-25-26  Chris Everett added this to allow some elements to be hidden, but to keep the data 

           case 'setoptioninactive':
            case 'setoptionactive': {
                $this->data->elementid = required_param('elementid', PARAM_INT);
                $this->data->optionid = required_param('optionid', PARAM_INT);
                break;
            }
 ...
 // 6-23-26 CE replaced the deleteelementoption completely
        if ($cmd == 'deleteelementoption') {

            // Delete an element option *********************************************************************.

            // 1. Delete the item and close the gap using our new engine
            ...
            // 3. Ensure errors array exists to prevent reference crashes, then draw the form
            $errors = [];
            $this->out .= $renderer->edit_option_form($this->cm, $this->data, 'add', $errors);
        ...
        // Check for usage using the Moodle database helper to handle column types
        $sql = "elementitemid = " . $DB->sql_compare_text('elementitemid') . " = ?";
        if ($DB->record_exists_sql("SELECT id FROM {tracker_issueattribute} WHERE elementitemid = ?", [$this->data->optionid])) {
                $this->out .= $OUTPUT->notification(get_string('errorcannotdeleteinuse', 'tracker'), 'notifyproblem');
            } else {
                // It's safe to delete
                $list = new datalist('tracker_elementitem', 'id', 'sortorder', ['elementid' => $this->data->elementid]);
                $list->remove($this->data->optionid);
...
// 6-25-26  CE added this to make elements active/or inactive so they don't show up if no longer needed
        if ($cmd == 'setoptioninactive' || $cmd == 'setoptionactive') {
            // Toggle an element option active state ****************************************.
            $active = ($cmd == 'setoptionactive') ? 1 : 0;
            
            // Update the database.
            $DB->set_field('tracker_elementitem', 'active', $active, ['id' => $this->data->optionid]);
